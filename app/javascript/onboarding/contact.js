...
export const maskInput = (element, pattern) => {
    switch (pattern) {
      ...
      default: {
        ...
        const rawValue = unmask(element.value, pattern);
        if (pattern === "(dd) ddddd-dddd") {
          if (rawValue.length === 11) {

            element.value = `(${rawValue.slice(0, 2)}) ${rawValue.slice(2, 7)}-${rawValue.slice(7)}`;
          } else if (rawValue.length === 10) {

            element.value = `(${rawValue.slice(0, 2)}) ${rawValue.slice(2, 6)}-${rawValue.slice(6)}`;
          }
        } else {
